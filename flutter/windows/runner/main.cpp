...
typedef char** (*FUNC_TEAMDESK_CORE_MAIN)(int*);
typedef void (*FUNC_TEAMDESK_FREE_ARGS)( char**, int);
typedef int (*FUNC_TEAMDESK_GET_APP_NAME)(wchar_t*, int);
...
{
  HINSTANCE hInstance = LoadLibraryA("libteamdesk.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libteamdesk.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_TEAMDESK_CORE_MAIN teamdesk_core_main =
      (FUNC_TEAMDESK_CORE_MAIN)GetProcAddress(hInstance, "teamdesk_core_main_args");
  if (!teamdesk_core_main)
  {
    std::cout << "Failed to get teamdesk_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_TEAMDESK_FREE_ARGS free_c_args =
      (FUNC_TEAMDESK_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = teamdesk_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
    ...
    // std::cout << "Teamdesk [" << args_str << "], core returns false, exiting without launching Flutter app." << std::endl;
    return EXIT_SUCCESS;
  }
  ...
  FUNC_TEAMDESK_GET_APP_NAME get_teamdesk_app_name = (FUNC_TEAMDESK_GET_APP_NAME)GetProcAddress(hInstance, "get_teamdesk_app_name");
  if (get_teamdesk_app_name) {
    wchar_t app_name_buffer[512] = {0};
    if (get_teamdesk_app_name(app_name_buffer, 512) == 0) {
