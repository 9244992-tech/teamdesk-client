...
class MainFlutterWindow: NSWindow {
    override func awakeFromNib() {
        ...
        let registrar = flutterViewController.registrar(forPlugin: "TeamdeskPlugin")
        ...
        FlutterMultiWindowPlugin.setOnWindowCreatedCallback { controller in
            ...
            self.setMethodHandler(registrar: controller.registrar(forPlugin: "TeamdeskPlugin"))
    ...
    private func enableNativeRelativeMouseMode(channel: FlutterMethodChannel) -> Bool {
        ...
        if result != CGError.success {
            NSLog("[Teamdesk] Failed to dissociate mouse from cursor position: %d", result.rawValue)
            return false
        }
        ...
        if state.eventMonitor == nil {
            NSLog("[Teamdesk] Failed to create event monitor for relative mouse mode")
    ...
    private func disableNativeRelativeMouseMode() {
        ...
        if result != CGError.success {
            NSLog("[Teamdesk] Failed to re-associate mouse with cursor position: %d, scheduling retry...", result.rawValue)
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.05) {
                ...
                if retryResult != CGError.success {
                    NSLog("[Teamdesk] Retry failed to re-associate mouse: %d. Cursor may remain locked.", retryResult.rawValue)
                }
            }
        }
    }
    ...
    public func setMethodHandler(registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(name: "org.teamdesk.teamdesk/host", binaryMessenger: registrar.messenger)
