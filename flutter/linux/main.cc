...
#define TEAMDESK_LIB_PATH "libteamdesk.so"
typedef bool (*TeamdeskCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_teamdesk_core_main() {
   void* libteamdesk = dlopen(TEAMDESK_LIB_PATH, RTLD_LAZY);
   if (!libteamdesk) {
      fprintf(stderr,"Failed to load \"libteamdesk.so\"\n");
      char* error;
   ...
   auto core_main = (TeamdeskCoreMain) dlsym(libteamdesk,"teamdesk_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"teamdesk_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
