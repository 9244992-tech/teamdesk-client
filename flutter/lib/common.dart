...
class MyTheme {
  ...
  static const Color accent = Color(0xFF1E63D8);
  static const Color accent50 = Color(0x771E63D8);
  static const Color accent80 = Color(0xAA1E63D8);
...
Future<void> windowOnTop(int? id) async {
  ...
  if (id == null) {
    ...
    await teamDeskWinManager.registerActiveWindow(kWindowMainId);
  } else {
    WindowController.fromWindowId(id)
      ..focus()
      ..show();
    teamDeskWinManager.call(WindowType.Main, kWindowEventShow, {"id": id});
  }
}
...
void msgBox(SessionID sessionId, String type, String title, String text,
  ...
  submit() {
    ...
    if (onSubmit != null) {
      ...
      // https://github.com/rustdesk/teamdesk/blob/5e9a31340b899822090a3731769ae79c6bf5f3e5/src/ui/common.tis#L263
...
Future<bool> restoreWindowPosition(WindowType type,
  ...
  if (bind
      .mainGetEnv(key: "DISABLE_TEAMDESK_RESTORE_WINDOW_POSITION")
      .isNotEmpty) {
    return false;
  }
  if (type != WindowType.Main && windowId == null) {
  ...
  if (lpos == null) {
    ...
    switch (type) {
      case WindowType.Main:
        ...
        // See https://github.com/rustdesk/teamdesk/blob/9b9276e7524523d7f667fefcd0694d981443df0e/flutter/macos/Runner/Base.lproj/MainMenu.xib#L333
      ...
      default:
        ...
        // https://github.com/rustdesk/teamdesk/blob/317639169359936f7f9f85ef445ec9774218772d/flutter/lib/utils/multi_window_manager.dart#L163
        break;
    }
    return true;
  }
      ...
      if (lpos.isMaximized == true) {
        ...
        if (isWindows) {
          if (storeSize) {
            ...
            // See known issue in https://github.com/rustdesk/teamdesk/pull/9840
  ...
  if (type != null && id != null) {
    switch (type) {
      case UriLinkType.remoteDesktop:
        Future.delayed(Duration.zero, () {
          teamDeskWinManager.newRemoteDesktop(id!,
              password: password,
      ...
      case UriLinkType.fileTransfer:
        Future.delayed(Duration.zero, () {
          teamDeskWinManager.newFileTransfer(id!,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.viewCamera:
        Future.delayed(Duration.zero, () {
          teamDeskWinManager.newViewCamera(id!,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.portForward:
        Future.delayed(Duration.zero, () {
          teamDeskWinManager.newPortForward(id!, false,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.rdp:
        Future.delayed(Duration.zero, () {
          teamDeskWinManager.newPortForward(id!, true,
              password: password, forceRelay: forceRelay);
        });
        break;
      case UriLinkType.terminal:
        Future.delayed(Duration.zero, () {
          teamDeskWinManager.newTerminal(id!,
              password: password, forceRelay: forceRelay);
        });
        break;
    }
...
connectMainDesktop(String id,
  ...
  if (isFileTransfer) {
    await teamDeskWinManager.newFileTransfer(id,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else if (isViewCamera) {
    await teamDeskWinManager.newViewCamera(id,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else if (isTcpTunneling || isRDP) {
    await teamDeskWinManager.newPortForward(id, isRDP,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else if (isTerminal) {
    await teamDeskWinManager.newTerminal(id,
        password: password,
        isSharedPassword: isSharedPassword,
        connToken: connToken,
        forceRelay: forceRelay);
  } else {
    await teamDeskWinManager.newRemoteDesktop(id,
        password: password,
        isSharedPassword: isSharedPassword,
        forceRelay: forceRelay);
  }
...
connect(BuildContext context, String id,
  ...
  if (isDesktop) {
    if (desktopType == DesktopType.main) {
      ...
      await teamDeskWinManager.call(WindowType.Main, kWindowConnect, {
        'id': id,
        'isFileTransfer': isFileTransfer,
        'isViewCamera': isViewCamera,
        'isTerminal': isTerminal,
...
Future<void> onActiveWindowChanged() async {
  print(
      "[MultiWindowHandler] active window changed: ${teamDeskWinManager.getActiveWindows()}");
  if (teamDeskWinManager.getActiveWindows().isEmpty) {
    // close all sub windows
    try {
      if (isLinux) {
        await Future.wait([
          saveWindowPosition(WindowType.Main),
          teamDeskWinManager.closeAllSubWindows()
        ]);
      } else {
        await teamDeskWinManager.closeAllSubWindows();
      }
    } catch (err) {
      debugPrintStack(label: "$err");
    } finally {
...
Future<List<Rect>> getScreenListWayland() async {
  ...
  if (isMainDesktopWindow) {
    ...
    final screenList = await teamDeskWinManager.call(
...
setNewConnectWindowFrame(int windowId, String peerId, int preSessionCount,
  ...
  if (screenRect == null) {
    ...
    // https://github.com/rustdesk/teamdesk/discussions/8825
      ...
      children: [
        Align(
          ...
          style: TextStyle(
            color: Colors.red,
            fontSize:
                18, // https://github.com/rustdesk/teamdesk-server-pro/issues/261
            fontWeight: FontWeight.bold,
          ),
...
// https://github.com/rustdesk/teamdesk/blob/ce1dac3b8613596b4d8ae981275f9335489eb935/flutter/lib/desktop/widgets/tabbar_widget.dart#L385
// https://github.com/rustdesk/teamdesk/blob/ce1dac3b8613596b4d8ae981275f9335489eb935/flutter/lib/desktop/widgets/tabbar_widget.dart#L399
//
// @platforms macos
...
String decode_http_response(http.Response resp) {
  try {
    // https://github.com/rustdesk/teamdesk-server-pro/discussions/758
