...
char g_lastMsg[1024];
const char* g_msgHeader = "TeamdeskIdd: ";
...
BOOL InstallUpdate(LPCWSTR fullInfPath, PBOOL rebootRequired)
{
    ...
    if (FALSE == UpdateDriverForPlugAndPlayDevicesW(
        NULL,
        L"TeamdeskIddDriver",    // match hardware id in the inf file
        fullInfPath,
        INSTALLFLAG_FORCE
    ...
    ))
    {
        ...
        if (error != 0)
        {
            ...
            {
            case 0x109:
                SetLastMsg("Failed InstallUpdate UpdateDriverForPlugAndPlayDevicesW, error: 0x%x, %s  Please try: Reinstall Teamdesk with the cert option.\n", error, errorString == NULL ? "(NULL)\n" : errorString);
                break;
            case 0xe0000247:
                SetLastMsg("Failed InstallUpdate UpdateDriverForPlugAndPlayDevicesW, error: 0x%x, %s  Please try: \n1. Check the device manager and event viewer.\n2. Uninstall \"TeamdeskIddDriver Device\" in device manager, then reinstall Teamdesk with the cert option.\n", error, errorString == NULL ? "(NULL)\n" : errorString);
                break;
            default:
...
BOOL DeviceCreateWithLifetime(SW_DEVICE_LIFETIME *lifetime, PHSWDEVICE hSwDevice)
{
    ...
    PCWSTR description = L"Teamdesk Idd Driver";

    // These match the Pnp id's in the inf file so OS will load the driver when the device is created
    PCWSTR instanceId = L"TeamdeskIddDriver";
    PCWSTR hardwareIds = L"TeamdeskIddDriver\0\0";
    PCWSTR compatibleIds = L"TeamdeskIddDriver\0\0";
    ...
    // Create the device
    HRESULT hr = SwDeviceCreate(L"TeamdeskIddDriver",
        L"HTREE\\ROOT\\0",
        &createInfo,
        0,
        NULL,
