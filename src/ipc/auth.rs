...
mod tests {
    ...
    fn test_executable_paths_match_windows_normalization() {
        let left = std::path::PathBuf::from(r"\\?\C:\Program Files\Teamdesk\Teamdesk.exe");
        let right = std::path::PathBuf::from(r"c:\program files\teamdesk\teamdesk.exe");
    ...
    fn test_os_str_eq_ignore_ascii_case_for_process_names() {
        assert!(super::os_str_eq_ignore_ascii_case(
            Some(std::ffi::OsStr::new("Teamdesk")),
            Some(std::ffi::OsStr::new("teamdesk"))
        ));
        assert!(!super::os_str_eq_ignore_ascii_case(
            Some(std::ffi::OsStr::new("Teamdesk")),
    ...
    fn test_portable_service_helper_trust_requires_content_match() {
        let unique = format!(
            "teamdesk-portable-helper-trust-test-{}-{}",
            std::process::id(),
    ...
    fn test_portable_service_helper_trust_accepts_matching_content() {
        let unique = format!(
            "teamdesk-portable-helper-trust-match-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap_or_default()
