...
// `teamdesk --deploy --token <api_token>` there is no point re-running the
...
impl RendezvousMediator {
        ...
        loop {
            ...
            *SOLVING_PK_MISMATCH.lock().await = "".to_owned();
            ...
            if !MANUAL_RESTARTED.load(Ordering::SeqCst) {
                ...
                // https://github.com/rustdesk/teamdesk/issues/12233
                sleep(0.033).await;
            }
        }
    }
        ...
        loop {
            ...
            select! {
                ...
                _ = timer.tick() => {
                    ...
                    // until the operator runs `teamdesk --deploy`.
            ...
            Some(rendezvous_message::Union::RegisterPkResponse(rpr)) => {
                ...
                match rpr.result.enum_value() {
                    ...
                    Ok(register_pk_response::Result::NOT_DEPLOYED) => {
                        if !NEEDS_DEPLOY.load(Ordering::SeqCst) {
                            log::warn!("Server requires deployment. Run `teamdesk --deploy --token <api_token>` on this device.");
                        }
    ...
    async fn register_pk(&mut self, socket: Sink<'_>) -> ResultType<()> {
        ...
        // `teamdesk --deploy --token <api_token>`.
        if NEEDS_DEPLOY.load(Ordering::SeqCst) {
