// https://github.com/rustdesk/teamdesk/discussions/6444#discussioncomment-9010062
...
//*************************************************************
//
//  DeleteTeamdeskTestCertsW_SingleHive()
//
//  Purpose:    Deletes Teamdesk Test certificates and wrong key stores
//
...
BOOL DeleteTeamdeskTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteTeamdeskTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteTeamdeskTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteTeamdeskTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteTeamdeskTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
