...
const MSG_TO_TEAMDESK_TARGET: &str = "teamdesk";
...
const MSG_TO_TEAMDESK_SIGNATURE_VERIFICATION: &str = "signature_verification";
...
pub(super) extern "C" fn cb_msg(
    ...
    match &target as _ {
        ...
        MSG_TO_TEAMDESK_TARGET => handle_msg_to_teamdesk(id, content, len),
        _ => PluginReturn::new(
...
fn handle_msg_to_teamdesk(id: String, content: *const c_void, len: usize) -> PluginReturn {
    ...
    match &msg_to_teamdesk.r#type as &str {
        MSG_TO_TEAMDESK_SIGNATURE_VERIFICATION => request_plugin_sign(id, msg_to_teamdesk),
        t => PluginReturn::new(
            errno::ERR_CALLBACK_TARGET_TYPE,
            &format!(
                "Unknown target type '{}' for target {}",
                t, MSG_TO_TEAMDESK_TARGET
            ),
        ),
    }
}
