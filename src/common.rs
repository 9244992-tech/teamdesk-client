...
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    let (request, url) =
        hbb_common::version_check_request(hbb_common::VER_TYPE_TEAMDESK_CLIENT.to_string());
...
fn get_api_server_(api: String, custom: String) -> String {
    ...
    let _ = custom;
    // Teamdesk: fixed Account-API endpoint (built-in login / address book).
    // Explicit `api-server` option (checked above) still overrides this.
    "https://api.teamdesk.su".to_owned()
}
